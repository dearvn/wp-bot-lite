...
namespace Dearvn\BotLite\Blocks;
...
class Manager {
	...
	public function register_all_blocks(): void {
        ...
        // Check if block API key provided yet or not.
        if ( ! wp_bot_lite()->settings->get()['enable_trading'] || empty( wp_bot_lite()->settings->get()['api_key'] ) || empty( wp_bot_lite()->settings->get()['secret_key'] ) ) {
            return;
        }
    ...
    protected function register_block_metas(): void {
        ...
        foreach ( $blocks as $block ) {
            ...
            $block_options   = [];
            $markup_file_path = BOT_LITE_TEMPLATE_PATH . '/blocks/' . $block . 'markup.php';
