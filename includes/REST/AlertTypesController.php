...
use Dearvn\BotLite\Alerts\AlertType;
use WP_REST_Response;
use WP_REST_Server;
use WP_Error;

/**
 * API AlertTypesController class.
 *
 * @since 0.3.0
 */
class AlertTypesController extends RESTController {
    ...
    public function get_items( $request ): ?WP_REST_Response {
        ...
        $alert_types = $alert_type->all( $args );

        foreach ( $alert_types as $alert_type_single ) {
            $response = $this->prepare_item_for_response( $alert_type_single, $request );
            $data[]   = $this->prepare_response_for_collection( $response );
        }

        $args['count'] = 1;
        $total         = $alert_type->all( $args );
